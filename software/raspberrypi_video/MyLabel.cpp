...
void MyLabel::mousePressEvent(QMouseEvent *event){
  if (event->button() == Qt::LeftButton) {
    ...
    std::cout << "-temperature "<< this->thread->getTempFromXY(lastPoint.x(),lastPoint.y()) << " ";
    ...
    std::cout << "-temperature " << this->thread->getTempFromArea(areaPointTopLeft.x(), areaPointTopLeft.y(), areaPointBottomRight.x(), areaPointBottomRight.y()) << " ";
  }
}
...
void MyLabel::keyPressEvent(QKeyEvent *event){
  ...
  if(event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter){
    ...
    // std::cout << "python3 test4.py ";
    
    // std::cout << "Timestamp: " << t->tm_mday << "-" << t->tm_mon << "-" << t->tm_year+1900 << " [" <<
    //             t->tm_hour << ":" << t->tm_min << ":" << t->tm_sec%60 << "]" << std::endl; 
    
    std::string command = "python3 test4.py -timestamp ";
    
    std::stringstream ss;
    ss << ((long int)now + 21600);
